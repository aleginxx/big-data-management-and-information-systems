...
Unlike earlier versions, it uses real semantic embeddings instead of random vectors and supports multiple similarity metrics (COSINE, L2, IP). 
The goal is to create collections that can be queried and benchmarked across different metrics.
"""
...
BATCH_SIZE = 32 # this is for generation of embeddings
...
categories = [str(row["label"]) for row in dataset]
...
print(F"Generated {subset_size} embeddings with dimension {vector_dim}")
